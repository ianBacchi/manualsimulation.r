...
simulate_pipeline <- function(instrucoes) {
  # Lista de instruções MIPS permitidas
  instrucoes_permitidas <- c("ADD", "SUB", "ADDI", "LW", "SW", "SLL", "BEQ", "BNE", "BLT", "BGT", "J")
  
  # Filtra instruções não permitidas
  instrucoes <- instrucoes[grep(paste(instrucoes_permitidas, collapse="|"), instrucoes)]
...
# Exemplo de uso da função simulate_pipeline com instruções permitidas
instrucoes <- c(
  "ADD $t0, $t1, $t2",
  "LW $t3, 100($t4)",
  "SUB $t5, $t6, $t7",
